...
const MessageContainer = ({onBackUser}) => {
  ...
  return (
    <div className='md:min-w-[500px] h-[99%] flex-col py-2'>
      ...
      <div className='flex justify-between gap-1 bg-sky-600 md:px-2 rounded-lg h-10 md:h-12'>
            ...
            <div className='flex-1 overflow-auto'>
              ...
              {!loading && messages?.length > 0 && messages?.map((message) => (
                ...
                <div className={`flex ${message.senderId === authUser._id ? 'justify-end' : 'justify-start' } my-2`}>
                  <div className={`max-w-[70%] px-3 py-2 rounded-lg ${message.senderId === authUser._id ? 'bg-sky-600 text-white' : 'bg-gray-300 text-black'}`}>
                    {message.message}
            ...
            <form onSubmit={ handelSubmit } className='rounded-full text-black'>
              <div className='w-full rounded-full flex items-center bg-white'>
                <input value={sendData} onChange={handelMessages} required id='message' type='text' className='w-full bg-transparent outline-none px-4 rounded-full'/>
                <button type='submit'>
...
//     <img src={message.senderId === authUser._id ? authUser.profilepic : selectedConversation.profilepic} className="w-10 rounded-full" />
//   </div>
//   <div className={`chat-bubble ${message.senderId === authUser._id ? 'bg-sky-600 text-white' : 'bg-gray-300 text-black'}`}>
//     {message.message}
//   </div>
//   <div className="chat-footer text-[10px] opacity-80">
//     {new Date(message.createdAt).toLocaleDateString('en-IN')}
